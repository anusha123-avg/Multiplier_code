// =============================================================================
// toom_cook_multiplier.v
//
// 3-way Toom-Cook multiplier, unsigned, parameterized WIDTH.
//
// This is the most involved of the five architectures - it generalises
// Karatsuba's "split in 2, use 3 multiplications instead of 4" trick to
// "split in 3, use 5 multiplications instead of 9", via an
// evaluate -> pointwise-multiply -> interpolate scheme (polynomial
// evaluation/interpolation), rather than Karatsuba's simpler algebraic
// identity.
//
// Each WIDTH-bit operand is treated as a degree-2 polynomial with 3 digits
// of THIRD = ceil(WIDTH/3) bits each:
//   A(x) = a2*x^2 + a1*x + a0     (x = 2^THIRD)
//
// Step 1 - EVALUATE both polynomials at 5 points: x = 0, 1, -1, 2, infinity
// (evaluating "at infinity" just means taking the leading coefficient, a2).
// Note A(-1) and B(-1) can come out negative, so this stage needs signed
// arithmetic even though the inputs/output are unsigned.
//
// Step 2 - MULTIPLY pointwise: 5 multiplications instead of the 9 a direct
// 3x3 digit expansion would need. This project uses plain Verilog signed
// multiplication for these (rather than routing through the custom
// schoolbook/Booth/Karatsuba modules), since the evaluated values are small,
// signed, and this is the natural way to express this interpolation-heavy
// stage.
//
// Step 3 - INTERPOLATE the 5 products back into the 5 coefficients c0..c4 of
// the full product polynomial C(x) = A(x)*B(x). This needs an exact division
// by 2 and an exact division by 3 partway through - both are guaranteed to
// divide evenly by the structure of the algorithm (verified in Python: see
// below), so plain integer division here is exact, not truncating/rounding.
// A highly-optimised implementation would replace the divide-by-3 with a
// cheaper multiply-by-modular-inverse trick instead of a real divider; this
// project uses a plain divider for clarity, and that area/complexity cost is
// exactly the overhead the literature review notes as the reason Rafferty et
// al. excluded Toom-Cook from their own hardware comparison.
//
// Step 4 - RECOMBINE: p = c0 + c1*2^THIRD + c2*2^(2*THIRD) + c3*2^(3*THIRD)
// + c4*2^(4*THIRD). c0..c4 are guaranteed non-negative (a product of two
// non-negative-coefficient polynomials always has non-negative coefficients),
// so they're safely reinterpreted as unsigned for this final step.
//
// This exact algorithm - including the two exact-division steps - was
// verified in Python first: exhaustively for widths 6, 8 and 9 (confirming
// both divisions are always exact with zero remainder), and against 50,000
// random cases each at widths 8, 16 and 32.
// =============================================================================
